...
class DataMapper implements IteratorAggregate {
			...
			if ( ! empty($this->id))
			{
				...
				foreach (array('has_many', 'has_one') as $type)
				{
					foreach ($this->{$type} as $model => $properties)
					{
						...
						{
							$data = array( $this->removed_field => $timestamp); // @modified $data = array($this_model . '_id' => NULL);
						...
						{
							// if there are delete's, check if we need to update the removed timestamp
							if (in_array($this->removed_field, $this->fields) && ! isset($data[$this->removed_field]))
							{
								// remove it now
								$data = array(
									'id'					=> $this->id,
									$this->removed_field	=> $timestamp
								);
				...
				$this->db->update($this->table, array( $this->removed_field => $timestamp ));
				// TODO
	...
	public function include_related($related_field, $fields = NULL, $append_name = TRUE, $instantiate = FALSE)
	{
		...
		foreach($rfs as $rf)
		{
			...
			{
				// @modified to default remove comment of the line below
